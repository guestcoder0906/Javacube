...
typedef struct {
    ...
    int count;
    int minClusterSize; // Minimum number of structures in a valid cluster
} ClusterRequirement;
...
ClusterRequirement clusterReq = {
    ...
    .minClusterSize = 3 //Added minimum cluster size
};
    ...
    if (clusterReq.enabled)
    {
        ...
        if (clusterCount >= 2)
        {
            ...
            for (int i = 0; i < clusterCount; i++)
            {
                ...
                if (groupSize < clusterReq.minClusterSize) //check for minimum cluster size
                {
                    free(indices);
                    continue;
                }
